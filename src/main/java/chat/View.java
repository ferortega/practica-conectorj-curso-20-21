...
public class View {
    ...
    private void showChatRoomMenu(long chatRoomId) throws SQLException {
        ...
        switch (option) {
            ...
            case 3: this.deleteSentMessages(chatRoomId);
                     break;
    ...
    private void deleteSentMessages(long chatRoomId) throws SQLException {
        System.out.println("Borrando mensajes...");

        int msg_borrados = controller.deleteMessages(username, chatRoomId);
        if (msg_borrados>0) System.out.println("Se han borrado "+ msg_borrados + " mensajes de "+username);
        else System.out.println("No se han encontrado mensajes en este chat de "+username);
